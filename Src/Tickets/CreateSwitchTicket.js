...
module.exports = async(guild, user, GuildDoc, TicketSchema) => {
  ...
  const TicketChannelSwitch = await guild.channels.create(`Ticket-${GuildDoc.TicketCount}-${user.username}-Switch`, {
    type: 'text',
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .setTimestamp();
  //const msg = await TicketChannelSwitch.send(`Reaja com ${TrashEmoji} para fechar o Ticket!`);
  const msg = await TicketChannelSwitch.send(embed);
  ...
  const TickDoc = new TicketSchema({
    ...
    GuildId: guild.id,
    UserId: user.id,
    ChannelId: TicketChannelSwitch.id,
    MessageId: msg.id
  })
